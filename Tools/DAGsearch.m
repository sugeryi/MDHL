function [Gmin_adjMatrix,scores,evals] = DAGsearch(X,maxIter,restartVal,complexityFactor,PP)
% Inputs:
%   X(instance,feature)
...
% XX = X(:,parents)'*X(:,parents);
% Xy = X(:,parents)'*(X(:,i));
% yy = X(:,i)'*X(:,i);
XX =sin(X(:,parents))'*sin(X(:,parents));
Xy =sin(X(:,parents))'*(X(:,i));
yy = sin(X(:,i))'*(X(:,i));
params = inv(XX)*Xy;
